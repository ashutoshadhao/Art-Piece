...
// Send OTP Email
export const sendOtpEmail = (userData) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.post(
      `${process.env.REACT_APP_API_URL}/api/v1/verify/sendmail`,
      userData,
      config
    );
...
export const verifyOTP = (userData) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.post(
      `${process.env.REACT_APP_API_URL}/api/v1/verify/user`,
      userData,
      config
    );
    console.log(data);
    const response = { message: data.message, status: data.status };
...
export const contact = (userData) => async (dispatch) => {
  try {
    ...
    dispatch({ type: CONTACT_USER_SUCCESS, payload: data });
  } catch (error) {
    dispatch({
...
export const contactDetails = () => async (dispatch) => {
  try {
    ...
    dispatch({ type: ALL_CONTACT_FAIL, payload: error.response.data.message });
  }
};
// Clearing Errors
