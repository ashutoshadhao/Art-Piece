...
export const getProduct =
  (keyword = "", currentPage = 1, price = [1, 125000], category, ratings = 0) =>
  async (dispatch) => {
    try {
...
export const getSellerProduct = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(
      `${process.env.REACT_APP_API_URL}/api/v1/products?user=${id}`
    );
...
export const createProduct = (productData) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.post(
      `${process.env.REACT_APP_API_URL}/api/v1/products/new`,
      productData,
      config
    );
...
export const updateProduct = (id, productData) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.put(
      `${process.env.REACT_APP_API_URL}/api/v1/seller/product/${id}`,
      productData,
      config
    );
...
export const getAllReviews = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(
      `${process.env.REACT_APP_API_URL}/api/v1/reviews?id=${id}`
    );
...
export const verifyProduct = (id) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(
      `${process.env.REACT_APP_API_URL}/api/v1/admin/verify/Product/${id}`
    );

    dispatch({
      type: VERIFY_PRODUCT_SUCCESS,
      payload: data.success,
    });
  } catch (error) {
    dispatch({
